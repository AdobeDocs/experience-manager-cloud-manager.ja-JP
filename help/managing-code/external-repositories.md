...
>[!TAB GitHub Enterprise]
    ...
    [ アクセストークンの管理 ] （/help/managing-code/manage-access-tokens.md）も参照。
...
>[!TAB GitLab]
    ...
    | — | — |
    | **既存のアクセストークンを使用** |組織のリポジトリアクセストークンを既に指定していて、複数のリポジトリにアクセスできる場合、既存のトークンを選択できます。 「**トークン名**」ドロップダウンリストを使用して、リポジトリに適用するトークンを選択します。 そうでない場合は、新しいアクセストークンを追加します。 |
    | **新しいアクセストークンの追加** |&lt;ul>&lt;li> 「**トークン名**」テキストフィールドに、作成するアクセストークンの名前を入力します。&lt;li>[GitLab ドキュメント ] （https://docs.gitlab.com/user/profile/personal_access_tokens/）の指示に従って、個人用アクセストークンを作成します。&lt;li>GitLab 個人用アクセストークン（PAT）に必要な権限 &lt;br> これらのスコープを使用すると、Cloud Managerは、検証と Webhook 統合のために必要に応じて、リポジトリデータとユーザー情報にアクセスできます。&lt;br>GitLab で PAT を生成する場合は、次のトークン範囲が含まれていることを確認します。&lt;ul>&lt;li>api&lt;li>read_user&lt;/li>&lt;/li>&lt;/ul>&lt;/li>&lt;/ul>&lt;/ul>&lt;ul>&lt;li>**Access Token** フィールドに、作成したトークンを貼り付けます。 |
