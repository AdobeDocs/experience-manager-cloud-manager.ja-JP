...
description: ここでは、Adobe Managed Services（AMS）用 Cloud Manager の概要およびクラウド内の Adobe Experience Manager を組織で自己管理する方法を説明します。
...
>
>AEM as a Cloud Service の同等のドキュメントについては、[AEM as a Cloud Service のドキュメント](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/home.html)を参照してください。
...
このような場合、[!UICONTROL Cloud Manager] は自動スケーリングプロビジョニング処理を自動的にトリガーし、自動スケーリングイベントの通知を送信し、数分以内に追加容量をオンラインにします。追加容量は実稼動環境の、実行中の Dispatcher/パブリッシュノードと同じシステム仕様に一致する同じリージョンでプロビジョニングされます。
