...
上記のように、パイプラインの一環として、コードがスキャンされます（現在は SonarQube で実装されています）。汎用の Java ルールと AEM 固有のルールを組み合わせた 93 個のルールがあります（Cognifide の既存のルールセットを含む）。A list of these rules can be found here: [code-quality-rules.xlsx](/help/using/assets/code-quality-rules.xlsx)
