...
* ステージのみへデプロイする場合は、パイプラインの **実稼動環境に昇格** ステップを拒否することによってのみ可能です。 ただし、実行はキャンセルとしてマークされます。
...
1. **パイプライン名**&#x200B;を指定します。ダイアログの残りのオプションと機能は、標準の結合パイプライン作成ダイアログと同じように動作します。「**保存**」をクリックして、パイプラインを保存します。
