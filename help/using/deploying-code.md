...
1. 完全なビルドプロセスによってコードがデプロイされます。
   ...
   * **実稼動へのデプロイ**
   ...
   >
   >
   >このオプションを使用すると、本番用の削除をスケジュールするか、「 **今すぐ」** をクリックして実稼動用の展開をすぐに実行できます。
   >
   >
   ...
   上記の手順で **「今すぐ」** オプションを選択すると、次の画面が表示されます。
...
1. ディスパッチャーのアーティファクトは、以下のように各ディスパッチャーにデプロイされます。
   ...
   1. すべての設定は、不変ファイルを除いて削除されます。詳しくは、「ディスパッチャー設定の管理」を参照してください。これにより、孤立したファイルが残らないようにディレクトリがクリアされます。
   ...
   >[!NOTE]
   >
   >Cloud Manager では、ディスパッチャーアーティファクトに完全なファイルセットが含まれていることが想定されています。すべてのディスパッチャー設定ファイルが、Git リポジトリに存在する必要があります。ファイルやフォルダーが見つからない場合、デプロイメントに失敗します。
