...
実稼動デプロイメント手順の再実行は、実稼動デプロイ手順が完了した実行に対してサポートされています。 完了のタイプは重要ではありません。デプロイメントは、成功（AMS プログラムの場合のみ）、キャンセル、失敗のいずれかの可能性があります。 ただし、主なユースケースは、一時的な理由で実稼動デプロイメント手順が失敗した場合であることが予想されます。 再実行の場合は、同じパイプラインを使用して新しい実行が作成されます。 この新しい実行は、次の 3 つのステップで構成されます。
...
* ロールバック実行には、再実行は使用できません。
* 最後の実行がロールバック実行である場合、再実行はできません。
