...
1. ターゲット環境で除外パスを削除または保持するよう選択できます。 チェックボックスを選択 `Do not delete exclude paths from destination` コンテンツセットで指定された除外パスを保持する場合。 チェックボックスがオフのままの場合、ターゲット環境では除外パスが削除されます。
