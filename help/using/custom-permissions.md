...
この節では、これらの手順について詳しく説明します。独自のカスタム権限を作成する際に、「[ 条件 ](#terms) および [ 設定可能な権限 ](#configurable-permissions) の節を確認すると便利な場合があります。
...
1. Admin Console の「**製品**」タブにリダイレクトされ、ここでユーザーと Cloud Manager の権限を管理できます。Admin Consoleで、「**新規プロファイル** ボタンをクリックします。
...
1. 権限を編集するには、権限の **編集** リンクをクリックします。
...
1. 権限項目の横にあるプラス（`+`）アイコンをクリックして、列 **含まれる権限項目** に追加します。

   * 詳しくは、権限項目の横にある「`i`」アイコンをクリックしてください。

1. **使用可能な権限** 列の上部にある「**すべてを追加** ボタンをクリックして、すべての権限を追加します。 同様に、「**すべてを削除**」をクリックすると、以前に選択したすべての権限が削除されます。
...
1. 「**ユーザーを追加**」ボタンをクリックし、カスタム権限を持つユーザーを新しい製品プロファイルに割り当てます。
...
| 用語 | 説明 |
|---|---|
| 定義済みの権限 | **ビジネス所有者**、**デプロイメントマネージャー**&#x200B;などの事前定義済みの役割。Cloud Manager の様々な機能を管理します。事前定義済みの役割について詳しくは、「[ 役割ベースの権限 ](/help/requirements/role-based-permissions.md)」を参照してください。 |
