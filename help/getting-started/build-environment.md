...
アップデートされたバージョンでスムーズなエクスペリエンスを実現するために、アドビでは、ユーザーが Mavenリポジトリを更新して HTTP ではなく HTTPS を使用することをお勧めします。 この調整は、安全な通信プロトコルに向けた業界の移行をサポートし、安全で信頼性の高いビルドプロセスを維持します。
